...
//   RESEND_API_KEY  — your Resend API key (send-only permission)
//   LEAD_TO         — destination email (defaults to gartexbuilders@gmail.com)
//   LEAD_FROM       — sender address. With gartex-construction.com
//                     verified in Resend: "Gartex Construction Company
//                     <noreply@gartex-construction.com>" or similar.
//                     For DNS-not-yet-configured hosts use the
//                     Resend-provided sandbox:
//                       "Gartex Website <onboarding@resend.dev>"
//
// Endpoint contract:
//   POST /api/lead    { name, phone?, email, location?, project }
//   → 200 { ok: true, lead_id }
//   → 400 { error }   when required fields missing
...
function isBotSafe(input) {
  // Strip HTML tags / scripts the easy way (this is also why we
  // render the email as plain text — never as HTML).
...
async function onRequestPostImpl({ request, env }) {
  ...
  if (body.website && body.website.trim() !== '') {
    return json({ ok: true }, 200);  // pretend OK, drop the lead
  }
  ...
  const from = (env && env.LEAD_FROM) || FROM_DEFAULT;
...
<div style="font-family:Inter,-apple-system,BlinkMacSystemFont,system-ui,sans-serif;line-height:1.55;color:#0b1729;">
  ...
  if (!res.ok) {
    ...
    return json({ error: `Resend rejected the email (${res.status}).`, detail: detail.slice(0, 300) }, 502);
  }
...
// Tiny escapers used by the HTML body — output text is the source of
// truth, HTML is just a nicer view in clients that render it.
function esc(s) {
